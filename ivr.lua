...
Вызов динамического ivr который хранится в реждисе. 
формируется по определенному алгоритму фронтендом а астериск уже достает значенияю 
Распарсивает их и считывает необходимые параметрыю 
Сам пример мало понятен будет без знания алгоритма, но примерно такие задачи я решаю на луа в диалплане 
(на goto можно не обращать вниманияю Просто это вреезка диалплана на lua в  обычный диалпланю 
Со временем уберу, так как это в общем то костыль)
